...
export class WebhookService {
    ...
    whatsappService;
    logger;
    constructor({ contactService, messageLogService, aiLogService, sessionService, classificationService, routingService, openaiService, whatsappService, logger, }) {
    ...
    async processSingleInboundMessage(message) {
        ...
        if (routeAction === RouteAction.SILENCE) {
            this.logger.info({ waId: message.waId, type: contact.type }, "Message silenced by router");
            return;
        }
        if (routeAction === RouteAction.ASK_DISAMBIGUATION) {
            ...
            await this.messageLogService.logOutbound(contact.id, DISAMBIGUATION_QUESTION, waResponse?.messages?.[0]?.id ?? null, { routeAction });
        ...
        if (routeAction === RouteAction.RESPOND_AI) {
            ...
            await this.messageLogService.logOutbound(contact.id, ai.text, waResponse?.messages?.[0]?.id ?? null, { routeAction });
            await this.sessionService.touchOutbound(contact.id);
        }
    }
}
